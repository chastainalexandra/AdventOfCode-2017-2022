...
class Updater {
    ...
    public async Task Update(int year, int day) {
        ...
        var requester = new DefaultHttpRequester("https://github.com/chastainalexandra/AdventOfCode-2017-2022");
        ...
        context.SetCookie(new AngleSharp.Dom.Url(baseAddress.ToString()), "session=" + session);
    ...
    private IBrowsingContext GetContext() {
        ...
        context.SetCookie(new AngleSharp.Dom.Url(GetBaseAddress().ToString()), "session=" + GetSession());
        return context;
    }
    ...
    async Task<Problem> DownloadProblem(IBrowsingContext context, Uri baseUri, int year, int day) {
        ...
        var input = await context.GetService<IDocumentLoader>().FetchAsync(
                new DocumentRequest(new AngleSharp.Dom.Url(baseUri + $"{year}/day/{day}/input"))).Task;
